...
class XlsxSource(is: () => InputStream, sheet: String) extends GraphStage[SourceShape[Row]] {
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic = new GraphStageLogic(shape) {
    ...
    private var sheetStream: InputStream = _
    ...
    override def preStart(): Unit = {
      try {
        ...
        case NonFatal(e) =>
          if (events ne null) try { events.close() } catch { case NonFatal(ex) => e.addSuppressed(ex) }
          if (sheetStream ne null) try { sheetStream.close() } catch { case NonFatal(ex) => e.addSuppressed(ex) }
          if (pkg ne null) try { pkg.close() } catch { case NonFatal(ex) => e.addSuppressed(ex) }
          if (stream ne null) try { stream.close() } catch { case NonFatal(ex) => e.addSuppressed(ex) }
          failStage(e)
      }
    }

    override def postStop(): Unit = {
      if (events ne null) try { events.close() } catch { case NonFatal(_) => }
      if (sheetStream ne null) try { sheetStream.close() } catch { case NonFatal(_) => }
      if (pkg ne null) try { pkg.close() } catch { case NonFatal(_) => }
      if (stream ne null) try { stream.close() } catch { case NonFatal(_) => }
    }
...
private[yuu] case class StreamingCell(address: CellAddress, valueType: String = null, style: CellStyle = null, value: String = null) extends Cell {

  private[this] var row: Row = _
  private[yuu] def withRow(r: Row): StreamingCell = {
    row = r
    this
  }
  ...
  override def getCellType(): Int = getCellTypeEnum match {
  ...
  override def getCellTypeEnum(): CellType = valueType match {
...
private[yuu] case class StreamingRow(num: Int, cells: Map[Int, StreamingCell] = Map.empty, style: CellStyle = null, defaultStyle: CellStyle = null) extends Row {
  ...
  override def getCell(cellnum: Int, policy: Row.MissingCellPolicy): Cell = policy match {
